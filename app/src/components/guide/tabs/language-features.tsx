...
const FeatureCard: React.FC<FeatureCardProps> = ({
  ...
  return (
    <div className="mb-3 sm:mb-5">
      ...
      >
        <div className="p-3 sm:p-4 flex items-center justify-between relative min-h-[56px] sm:min-h-auto">
          <div className="flex items-center gap-2 sm:gap-3 relative z-10">
            <div className="text-tokyo-green bg-tokyo-green/10 p-1.5 sm:p-2 rounded-lg">
              {icon}
            </div>
            <h3 className="font-medium text-[var(--tokyo-fg)] text-sm sm:text-base">
              {title}
            </h3>
          </div>
          <ChevronRight
            size={16}
            className={`text-[var(--tokyo-fg-dark)] transition-transform duration-300 sm:w-[18px] sm:h-[18px] ${
              expanded ? "rotate-90" : ""
            }`}
          />
        </div>
        ...
        {expanded && (
          <div className="px-3 sm:px-4 pb-4 sm:pb-5 pt-1 border-t border-tokyo-comment/30 relative overflow-hidden">
            <p className="text-[var(--tokyo-fg)] mb-3 sm:mb-4 leading-relaxed relative z-10 text-sm sm:text-base">
              {description}
            </p>
