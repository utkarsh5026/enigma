    ...
    <div className="space-y-8">
      {Object.entries(categories).map(([category, nodeTypes]) => (
        <div key={category} className="relative">
          <div className="ml-4">
            <h3 className="text-lg font-medium text-tokyo-fg mb-4 flex items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {nodeTypes.map((nodeType) => (
                <div
                  key={nodeType}
                  className="bg-gradient-to-br from-tokyo-bg-dark to-tokyo-bg-dark/70 border-none rounded-lg p-4 backdrop-blur-sm hover:border-tokyo-comment/70 transition-colors duration-300 relative overflow-hidden group"
                >
                  <div className="relative z-10">
                    ...
                    <p className="text-[var(--tokyo-fg)] text-sm mb-3 leading-relaxed">
                      {nodeDescriptions[nodeType]}
                    </p>
                    {nodeExamples[nodeType] && (
                      <div className="bg-tokyo-bg-dark/80 rounded-md p-3 text-xs font-mono border border-tokyo-comment/30 backdrop-blur-sm">
                        <ScrollArea className="h-[100px]" dir="ltr">
...
const ASTNodeCard: React.FC<ASTNodeCardProps> = ({
  ...
  return (
    <div className="bg-tokyo-bg-dark/60 p-4 rounded-lg border-none backdrop-blur-md relative overflow-hidden">
      <div
        className={`absolute -top-4 -left-4 w-12 h-12 ${getColorClass(
          type
        )} rounded-full filter blur-xl`}
      ></div>
      <div className="relative z-10">
        <div className="flex items-center gap-2 mb-2">
          <Badge className="bg-tokyo-bg-dark text-tokyo-fg backdrop-blur-sm">
            {title}
          </Badge>
        </div>
        <p className="text-[var(--tokyo-fg)] text-sm leading-relaxed">
          {description}
        </p>
      </div>
    </div>
...
const SyntaxReference = () => {
  ...
  return (
    <div>
      <div className="mb-8 bg-gradient-to-br from-tokyo-bg-dark to-tokyo-bg-dark/60 rounded-xl p-6 border-none backdrop-blur-md relative overflow-hidden">
        <h2 className="text-xl font-bold text-tokyo-fg mb-3 relative z-10">
          Syntax Reference
        </h2>
        <p className="text-[var(--tokyo-fg)] leading-relaxed relative z-10">
      ...
      <div className="mt-10 bg-gradient-to-br from-[var(--tokyo-bg-dark)]/80 to-[var(--tokyo-bg-dark)]/60 rounded-xl p-6 border border-[var(--tokyo-comment)]/50 backdrop-blur-md">
        <h3 className="text-lg font-medium text-[var(--tokyo-fg)] mb-3">
